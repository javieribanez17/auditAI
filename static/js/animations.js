...
if (document.getElementById("btn-upload")) {
  ...
  uploadFile.addEventListener("change", () => {
    btnFile.disabled =
      uploadFile.files.length === 0 || uploadFile.files.length > 2;
  });
...
function displaySelectedFiles() {
  ...
  if (inputElement.files.length > 0 && inputElement.files.length < 3) {
    ...
    fileNamesElement.innerHTML = `Archivos seleccionados: ${fileNames.join(
      ", "
    )}`;
  } else {
    // Si no se han seleccionado archivos, borrar el contenido
    fileNamesElement.innerHTML = "Selecciona máximo dos archivos";
  }
}
